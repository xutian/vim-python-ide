...
def _get_shortcuts(env, shortcuts, insert_shortcuts):
    """ Enable to define custom shortcuts by user .vimrc """
    rope_shortcuts = env.get('shortcuts')
    print rope_shortcuts
    if rope_shortcuts:
        for item in project+assists+refactor:
            if rope_shortcuts.get(item):
                if item in insert_shortcuts:
                    insert_shortcuts[item] = rope_shortcuts[item]
                else:
                    shortcuts[item] = rope_shortcuts[item]
...
def _add_menu(env):
    def construct_menu_item():
        if shortcuts.get(name):
            item = '\ '.join((name.replace('_', '\ ').title(), '\ ', str(shortcuts.get(name))))
        else:
...
_interface.init()
_get_shortcuts(_env, shortcuts, insert_shortcuts)
